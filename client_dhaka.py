...
c = socketio.Client()
c.connect("http://communication.dhaka.com:9000", namespaces=['/communication'])
...
def message(data):
    ...
    data = {'rating': rating, 'location': 'dhaka'}

    r = requests.post(url="http://server.dhaka.com:8005/rating", data=data)
...
def request_to_server():
    for i in range(10):
        ...
        r = requests.post(url="http://server.dhaka.com:8005/rider", data=data)
        ...
        #print(data)
    #
        r = requests.post(url="http://server.dhaka.com:8005/driver", data=data)
