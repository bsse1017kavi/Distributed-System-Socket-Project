...
location = os.environ['SERVERLOCATION']
...
def match_driver_rider():
    ...
    if len(riders) > 0 and len(drivers) > 0:
        for i in range((len(riders))):
            ...
            if location=='dhaka':
                r = requests.post('http://communication-dhaka:9000/message', data=data)
            else:
                r = requests.post('http://communication-chittagong:9000/message', data=data)
